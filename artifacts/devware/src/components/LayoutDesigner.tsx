...
export default function LayoutDesigner() {
  ...
  const preview = getPreviewCells(selected, autoRotated, hoverCell);
  const canDrop = hoverCell ? canPlace(layout.drills, selected, hoverCell[0], hoverCell[1], autoRotated) : false;
  ...
  const handleCellClick = (col: number, row: number) => {
    ...
    // Auto-pick rotation: use default unless only rotated fits
    const useRotated = isRotatable(selected)
      && !canPlace(layout.drills, selected, col, row, false)
      && canPlace(layout.drills, selected, col, row, true);
      ...
      <div>
        ...
        <div className="flex items-center justify-between mt-2">
          <p className="text-xs text-muted-foreground/40 font-mono">
            ...
            {isRotatable(selected) && <span className="text-muted-foreground/25 ml-1">· auto-rotates to fit</span>}
          </p>
        </div>
      </div>
